...
class AIPipelineService:
    def __init__(self, app, client_factory: Callable[[Any], Any] = get_supabase_client):
        self.app = app
        self.client_factory = client_factory

    def _build_cases_query(
        self,
        client,
    ...
    def train(self, filters: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        filters = filters or {}
        ...
        client = self.client_factory(self.app)
        query = self._build_cases_query(client, disease, start_date, end_date, verified_only)
        if query is None:
            model = train_baseline_pipeline([], model_version=self.app.config.get("AI_MODEL_VERSION", "baseline-v1"))
            self.app.extensions[APP_MODEL_KEY] = model
            return {
                "model": model,
                "validation": {
                    "rows": [],
        ...
        model = train_baseline_pipeline(
            normalized["rows"],
            model_version=self.app.config.get("AI_MODEL_VERSION", "baseline-v1"),
        )
        self.app.extensions[APP_MODEL_KEY] = model
        return {
            "model": model,
    ...
    def get_or_train(self, filters: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        if self.app.config.get("AI_AUTO_TRAIN_ON_READ", True):
            return self.train(filters=filters)

        cached = self.app.extensions.get(APP_MODEL_KEY)
        if cached is None:
            return self.train(filters=filters)
...
def train_ai_pipeline(
    app,
    filters: Optional[Dict[str, Any]] = None,
    client_factory: Callable[[Any], Any] = get_supabase_client,
) -> Dict[str, Any]:
    return AIPipelineService(app, client_factory=client_factory).train(filters=filters)


def get_or_train_ai_pipeline(
    app,
    filters: Optional[Dict[str, Any]] = None,
    client_factory: Callable[[Any], Any] = get_supabase_client,
) -> Dict[str, Any]:
    return AIPipelineService(app, client_factory=client_factory).get_or_train(filters=filters)
